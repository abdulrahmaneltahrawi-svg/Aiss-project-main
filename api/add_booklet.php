...
// منع التكرار
$checkSlug = $conn->prepare("SELECT id FROM booklets WHERE slug = ?");
...
$imagesDir = dirname(__DIR__) . "/assets/uploads/booklets/images/";
if (!is_dir($imagesDir)) {
...
$coverPath = "assets/uploads/booklets/images/" . $imageName;
...
$filesDir = dirname(__DIR__) . "/assets/uploads/booklets/files/";
if (!is_dir($filesDir)) {
...
$pdfPath = "assets/uploads/booklets/files/" . $pdfName;

// ===== حفظ في الداتا بيز =====
$stmt = $conn->prepare("
    INSERT INTO booklets (title, slug, cover_image, file_path)
    VALUES (?, ?, ?, ?)
");
...
if ($stmt->execute()) {
    respond(true, "Booklet added successfully", [
        "booklet_id" => $stmt->insert_id
    ]);
} else {
    respond(false, "Failed to add booklet");
}